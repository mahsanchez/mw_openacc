...
#pragma acc data copyin(state[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS ], state_tmp[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS ], flux[0:(nx + 1) * (nz + 1) * NUM_VARS], tend[0:nx * nz * NUM_VARS ], hy_dens_theta_cell[0:nz + 2 * hs], hy_pressure_int[0:nz + 1], hy_dens_cell[0:nz + 2 * hs])
    {
...
void semi_discrete_step(double *state_init, double *state_forcing, double *state_out, double dt, int dir, double *flux, double *tend)
{
    ...
    nvtxRangePushA("apply_tendencies_fluidstate");

    for (ll = 0; ll < NUM_VARS; ll++)
    {
#pragma acc parallel loop collapse(2) private(indt, inds) present( state_init[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS], state_out[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS], tend[0:nx * nz * NUM_VARS ])  vector_length(32) async(ll)
        for (k = 0; k < nz; k++)
        {
            for (i = 0; i < nx; i++)
            {
...
void compute_tendencies_x(double * restrict state, double * restrict flux, double * restrict tend)
{
...
#pragma acc parallel loop gang collapse(2) private(inds, r, u, w, t, p, stencil, vals, d3_vals) present( state[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS], flux[0:(nx + 1) * (nz + 1) * NUM_VARS], hy_dens_theta_cell[0:nz + 2 * hs]) vector_length(32)
    for (k = 0; k < nz; k++)
    {
        for (i = 0; i < nx + 1; i++)
        {
    ...
    for (ll = 0; ll < NUM_VARS; ll++)
    {
#pragma acc parallel loop collapse(2) private(indt, indf1, indf2) present(tend[0:nx * nz * NUM_VARS], flux[0:(nx + 1) * (nz + 1) * NUM_VARS]) vector_length(32)  async(ll)
        for (k = 0; k < nz; k++)
        {
            for (i = 0; i < nx; i++)
            {
...
#pragma acc parallel loop gang collapse(2) private(inds, r, u, w, t, p, stencil, vals, d3_vals) present(state[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS], flux[0:(nx + 1) * (nz + 1) * NUM_VARS], hy_pressure_int[0:nz + 1])  vector_length(32)
    for (k = 0; k < nz + 1; k++)
    {
        for (i = 0; i < nx; i++)
        {
    ...
    for (ll = 0; ll < NUM_VARS; ll++)
    {
#pragma acc parallel loop collapse(2) private(indt, indf1, indf2, inds) present(state[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS ], flux[0:(nx + 1) * (nz + 1) * NUM_VARS], tend[0:nx * nz * NUM_VARS ])  vector_length(32)  async(ll)
        for (k = 0; k < nz; k++)
        {
            for (i = 0; i < nx; i++)
            {
...
void set_halo_values_x(double *state)
{
    ...
    for (ll = 0; ll < NUM_VARS; ll++)
    {
#pragma acc parallel loop present(state[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS ])  vector_length(32) async(ll)
        for (k = 0; k < nz; k++)
        {
...
#pragma acc wait
    ...
    if (myrank == 0)
    {
#pragma acc parallel loop collapse(2) private(ind_r, ind_u, ind_t) present(state[0:(nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS ], hy_dens_theta_cell[0:nz + 2 * hs], hy_dens_cell[0:nz + 2 * hs]) vector_length(32)
        for (k = 0; k < nz; k++)
        {
            for (i = 0; i < hs; i++)
            {
