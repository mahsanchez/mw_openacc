...
#pragma acc parallel loop private(indt, inds)
...
#pragma acc parallel loop collapse(2) private(inds, r, u, w, t, p, stencil, vals, d3_vals)
    for (k = 0; k < nz; k++)
    {
        for (i = 0; i < nx + 1; i++)
        {
...
#pragma acc loop vector
    ...
    if (myrank == 0)
    {
#pragma acc parallel loop private(ind_r, ind_u, ind_t)  vector_length(256)
        for (k = 0; k < nz; k++)
        {
            for (i = 0; i < hs; i++)
            {
