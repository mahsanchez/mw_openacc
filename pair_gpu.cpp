...
void pair_gpu(const double * d_x, const double * d_y, const double * d_z,
              unsigned long long int *restrict d_g2, int numatm, int nconf,
...
#pragma acc data copy( h_g2[:sizebin] ) copyin( h_x[:nconf*numatm], h_y[:nconf*numatm], h_z[:nconf*numatm] )
    {
...
#pragma acc data  present( d_g2[:d_bin] ) present( d_x[:nconf*numatm], d_y[:nconf*numatm], d_z[:nconf*numatm] )
    //#pragma acc parallel num_gangs(10) num_workers(16) vector_length(32) present( d_g2[:d_bin] ) present( d_x[:nconf*numatm], d_y[:nconf*numatm], d_z[:nconf*numatm] )
    for (int frame = 0; frame < nconf; frame++)
    {
        //#pragma acc loop worker
#pragma acc parallel loop async(frame)
